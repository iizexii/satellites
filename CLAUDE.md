...
1. **Получение входных данных:** docx с текстом + ТЗ в чат
2. **THINK:** продумать структуру, сверить с criticals
3. **Парсинг docx** → извлечение title, description, H1, секций, FAQ
4. **Design skill** (`~/.claude/skills/frontend-design`) для визуала
5. **Генерация HTML** с контентом из docx (SEO-compliant)
6. **Генерация CSS** (mobile-first, BEM)
7. **Генерация JS** (burger, accordion)
8. **Генерация** robots.txt, sitemap.xml, .htaccess
9. **QA checklist** — финальная проверка
10. **Output** готовые файлы
11. **Push to GitHub** (если попросят)